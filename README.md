...
本项目从准确性、性能和用户体验三个维度进行离线评测。

| 维度 | 指标 | 结果 | 计算方式 |
| --- | --- | ---: | --- |
| 准确性 | 意图识别准确率 | 90%+ | 意图列表与人工标注完全一致的样本数 ÷ 1000；关键词匹配基线为 65% |
| 准确性 | 知识库问答准确率 | 95% | 知识问答子集中，回答关键事实与标准答案一致的数量 ÷ 该子集总数 |
| 准确性 | 用户偏好记忆准确率 | 95% | 偏好被正确追加或覆盖的样本数 ÷ 偏好相关样本总数 |
| 性能 | 端到端响应时间 | 约 30 秒 → 约 15 秒 | 从用户发送 query 到完整结果返回的平均耗时；优化前后使用同一批复合意图请求 |
| 性能 | Redis 缓存命中率 | 85% | 直接从 Redis 读取到偏好的次数 ÷ 偏好查询总次数 |

### 1000 条测试集

- **来源：** 由 ChatGPT 5.6 辅助生成，再由人工完成标签标注和复核。
- **范围：** 覆盖行程规划、偏好管理、记忆查询、知识问答和信息查询 5 个核心业务意图，每类约 200 条。
- **表达类型：** 包含口语化表达、同义表达、上下文依赖和复合意图。
- **意图判定：** 逐条调用 IntentionAgent，将输出的意图列表与人工标注对比；完全匹配记为正确，否则记为错误。
- **偏好判定：** 从测试集中筛选约 200 条偏好相关 query，执行完整流程，检查长期记忆中的追加、覆盖动作及最终字段值是否符合标注。
...
仓库中的 [小样本工程验证记录](tests/results/small_eval_20260822.md) 用于开发阶段的功能检查；上表为 1000 条测试集的离线评测结果，两者用途不同。
